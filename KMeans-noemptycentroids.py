...
def Randomize_Centroid_Iteration_1(dataSet,K):
	...
	for i in range(0,K):
		flag = False
		point_1 = random.choice(dataSet)
		point_2 = random.choice(dataSet)
		key = Datapoints((point_1.X_cordinate+point_2.X_cordinate)/2,(point_1.Y_cordinate+point_2.Y_cordinate)/2)
		for val in new_centroid_dict:
			if(val.X_cordinate == key.X_cordinate and val.Y_cordinate == key.Y_cordinate):
				flag = True
		if(flag):
			print ("Entering Flag")
			i = i -1
...
def kmeans(dataSet,K):
	...
	newVersion_Centroids = copy.deepcopy(Randomize_Centroid_Iteration_1(dataSet,K))
	newCentroid_dict = {}
	for i in range(2):
